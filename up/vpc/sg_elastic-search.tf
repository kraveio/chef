...
# all communication between all node types is done using the transport module TCP(9300) (with the exception 
# of the multicast protocol UDP(54328). The multicast protocol is NOT recommended for production.
...
resource "aws_security_group" "elasticsearch_zen_multicast" {
	vpc_id = "${aws_vpc.main.id}"
	name = "elasticsearch-zen-multicast"
...
resource "aws_security_group" "elasticsearch_couchbase_client" {
	vpc_id = "${aws_vpc.main.id}"
	name = "elasticsearch-couchbase-client"
	description = "Elastic Search Client - Marker Group"
}

resource "aws_security_group" "elasticsearch_couchbase_plugin" {
	vpc_id = "${aws_vpc.main.id}"
	name = "elasticsearch-couchbase-plugin"
	description = "Elastic Search Couchbase Plugin Port"
	...
	ingress {
		...
		to_port = 9091
		protocol = "tcp"
		security_groups = ["${aws_security_group.elasticsearch_couchbase_client.id}"]
	}
}
